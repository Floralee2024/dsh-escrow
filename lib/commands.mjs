...
const HELP = `escrow 用法：
  ...
  /escrow approve <id>        批准单个托管动作（放行执行，异步重放）
 ...
 */
function learnDecision(taste, entry, approved, { learnWhitelist, autoBlacklist }) {
  if (!taste || !entry?.tasteSignature) return null;
  if (approved && !learnWhitelist) return null;
  if (!approved && !autoBlacklist) return null;
  return taste.recordDecision(entry.tasteSignature, approved, { toolName: entry.tool, args: entry.argsSnapshot ?? {} });
}

export function registerEscrowCommands(ctx, queue, ledger, taste = null, opts = {}) {
  const { learnWhitelist = true, autoBlacklist = true, repeatThreshold = 10, pluginHash = '', rules = [] } = opts;
  ...
  ctx.commands.register({
    ...
    input: { hint: 'pending | approve <id|all> | deny <id|sig|all> | allowlist | allow <sig> | export | import <path> | stats | help' },
    recordInput: true,
    handler(invocation) {
      ...
      switch (cmd) {
        ...
        case 'allow': {
          ...
          if (!taste) return error('[escrow] 品味习得未启用');
          taste.allow(arg);
          return success(`[escrow] 已手动加入白名单（立即生效）：${arg}`);
        }
        case 'forget': {
