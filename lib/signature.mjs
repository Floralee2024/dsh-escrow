...
import { SHELL_TOOLS, collectPathCandidates, SELFMOD_PATTERNS, findCommandSelfMod, GIT_GLOBAL_OPT, BUILTIN_EXTERNAL_COMMAND_PATTERNS, getTrustedEffects } from './classify.mjs';
 ...
 * 外部发布/权限/共享资源动作、普通 `git push` 与绝对破坏性命令永不自动学习。
 * @returns {string|null} 命中原因；null = 可学习。
 */
const NEVER_LEARN_COMMAND_PATTERNS = [
  // 任何远端推送都需要每次明确确认；强制推送另有 critical-red 分类。
  new RegExp(String.raw`\bgit(?:\.exe)?${GIT_GLOBAL_OPT}\s+push\b`, 'i'),
 ...
 * @returns {string|null} 命中原因；null = 可学习。
 */
export function neverLearnReason(toolName, args, meta = {}) {
  const command = typeof args?.command === 'string' ? args.command : '';
  if (command) {
    if (NEVER_LEARN_COMMAND_PATTERNS.some((r) => r.test(command))) {
      if (/\bgit(?:\.exe)?[^\n]*\s+push\b/i.test(command)) return 'git push 外部推送动作永不自动学习';
    ...
    if (external) return `外部副作用/权限动作（${external.label}）永不自动学习`;
  }
  const trusted = getTrustedEffects({ name: toolName, trustedEffects: meta.trustedEffects }, {});
  if (trusted.length > 0) return `可信副作用标签（${trusted.join('、')}）永不自动学习`;
