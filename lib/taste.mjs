...
export function createTasteStore({ dir, threshold = 2, cooldownHours = 24, now = () => Date.now(), pluginHash = '' } = {}) {
  ...
  return {
     ...
     * @param {object} opts - { toolName, args } 原始工具调用（never-learn 判定依据）
     * @returns {{learned:boolean, reason?:string, status?:string}}
     */
    recordDecision(signature, approved, { toolName = '', args = {}, assumeLearnable = false } = {}) {
      if (!assumeLearnable) {
        const nl = neverLearnReason(toolName, args);
      ...
      if (entry.status === 'learning' && entry.count >= threshold) {
        if (approved) {
          ...
          entry.cooldownUntil = now() + cooldownHours * 3600 * 1000;
        } else {
