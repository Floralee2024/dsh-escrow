...
export function findCommandSelfMod(command) {
  ...
  // glob 通配（AGENTS.m?）/ 变量（$F.md）属确定性检测固有上限（shell 展开后无法静态解析），接受。
  const stripped = command.replace(/['"]/g, '');
  const hit = SELFMOD_TARGET_MARKERS.find((re) => re.test(stripped));
...
/**
 * 把扁平规则转成内部结构。每条规则：
 * { id, tools: string[], args: {key,pattern}[], paths: string[], action }
 */
...
export function classifyExec(exec, opts = {}) {
  ...
  for (const rule of rules) {
    ...
    return { action: hit.action, ruleId: hit.id || '(user)', source: 'user', reason: ruleReason(hit, name, command, args) };
  }
  ...
  if (builtinRules) {
    ...
    if (selfModRed) {
      ...
      if (smPath) return { action: 'red', ruleId: 'selfmod', source: 'builtin', reason: `自改路径: ${smPath.label}` };
      if (command) {
        const smCmd = findCommandSelfMod(command);
        if (smCmd) return { action: 'red', ruleId: 'selfmod', source: 'builtin', reason: `命令串写入自改目标: ${smCmd}` };
      }
    }
  }
