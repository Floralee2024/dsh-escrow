...
export function createQueue({ ttlSec = 30, timeoutPolicy = 'cancel', now = () => Date.now() } = {}) {
  ...
  function snapshot(id) {
    ...
    return {
      ...
      approvalPlan: entry.approvalPlan,
   ...
   */
  function enqueue({ tool, reason, signal, summary, signatureHash: sig, originalCallId, argsSnapshot, agent, onSettle, tasteSignature, sync, snapshots, selfmod, trustedEffects, riskClass, actionDetails, approvalPlan, neverLearnReason } = {}) {
    ...
    const entry = {
      ...
      trustedEffects: Array.isArray(trustedEffects) ? [...trustedEffects] : [], // 不进入 args/signature，只用于 never-learn
      neverLearnReason: neverLearnReason || null,
      riskClass: riskClass || 'red',
      actionDetails: actionDetails && typeof actionDetails === 'object' ? { ...actionDetails } : {},
      approvalPlan: approvalPlan && typeof approvalPlan === 'object' ? { ...approvalPlan } : {},
      state: 'pending'
    };
   ...
   */
  function decide(id, decision, via = 'human') {
    const entry = entries.get(id);
    if (!entry || entry.settled) return false;
    entry.settle(decision === 'approved' ? 'approved' : 'denied', via);
    return true;
  }
