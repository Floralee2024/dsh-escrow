...
import { classifyExec, normalizeAction, collectPathCandidates, extractRedirectTargets } from './classify.mjs';
import { createLedger } from './ledger.mjs';
import { createQueue, signatureHash, normalizeTimeoutPolicy } from './queue.mjs';
import { registerEscrowCommands } from './commands.mjs';
...
export const Config = z.object({
  ...
  /** 用户规则（first match wins，但 green 不能压制内置红灯）：{id?, tools[], args[{key,pattern}], paths[], action} */
...
export function apply(ctx, config) {
  ...
  ctx.logger?.info(`[escrow] ttl=${ttlSec}s policy=${policy} default=${defaultAction} mode=${mode} syncTools=${syncTools.length ? syncTools.join(',') : '-'} taste=${learnWhitelist ? `on(th=${learnThreshold})` : 'off'} blacklist=${autoBlacklist ? 'on' : 'off'} ledger=${ledger.path}`);
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    try {
      ...
      if (t === 'allow') {
        ...
        const cls = classifyExec(exec, { rules, builtinRules, defaultAction, selfModification: selfModRed });
        const hardRed = cls.action === 'red' && (
          cls.source === 'user'
          || cls.ruleId === 'builtin-sensitive-path' || cls.ruleId === 'builtin-command-sensitive' || cls.ruleId === 'selfmod'
        );
        if (!hardRed) {
    ...
    let queuedId = null;
    try {
      const classified = classifyExec(exec, { rules, builtinRules, defaultAction, selfModification: selfModRed });
      ...
      if (cls === 'yellow') {
        ...
        const isSyncTool = mode === 'sync' || syncTools.includes(exec?.name);
        ...
        if (isSyncTool) {
          ...
          const { id, promise, dedup } = queue.enqueue({
            ...
            tasteSignature: m2Sig, sync: true, snapshots, selfmod: isSelfMod
          });
          queuedId = id;
          ...
          const { id, dedup } = queue.enqueue({
            tool: exec?.name, reason, signal: exec?.signal, summary: summarize(safeArgs),
            signatureHash: sig, originalCallId: callId, argsSnapshot, agent: exec?.agent,
            tasteSignature: m2Sig, sync: false, snapshots, selfmod: isSelfMod,
  ...
  registerEscrowCommands(ctx, queue, ledger, taste, { learnWhitelist, autoBlacklist, learnThreshold, repeatThreshold, pluginHash, rules });
